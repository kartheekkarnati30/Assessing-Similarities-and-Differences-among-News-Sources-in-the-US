...
require(purrr)
project_data<-fread("../ArticleData.csv")
problems <- project_data[,.(num_script_problems = sum(grepl("(jQuery|containerid)",article_text))), by = news_site]
...
remaining_problems <-project_data2[,.(num_script_problems_old = sum(grepl("(jQuery|containerid)",article_text)),
